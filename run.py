...
## Free Cash Flow
depamor, ttm_depamor, _, ann_depamor, _ = fa.get_fin_state_data('Total Depreciation And Amortization - Cash Flow', 'sum', ttm=True)
assliachg, ttm_assliachg, _, ann_assliachg, _ = fa.get_fin_state_data('Total Change In Assets/Liabilities', 'sum', ttm=True)
ppe, ttm_ppe, _, ann_ppe, _ = fa.get_fin_state_data('Net Change In Property, Plant, And Equipment', 'sum', ttm=True)
intax, ttm_intax, _, ann_intax, _ = fa.get_fin_state_data('Income Taxes', 'sum', ttm=True)
#
fcf = net + depamor + assliachg + ppe + intax
ttm_fcf = ttm_net + ttm_depamor + ttm_assliachg + ttm_ppe + ttm_intax
ann_fcf = ann_net + ann_depamor + ann_assliachg + ann_ppe + ann_intax
#
fn = 'Free Cash Flow (Q)'
fig = plt.figure(dpi=150, figsize=(15, 5))
ax = plt.axes()
